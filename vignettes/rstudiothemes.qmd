...
**Positron** and **TextMate** theme files to **RStudio** `.rstheme` files and
install them in your **RStudio** IDE. It also includes bundled **RStudio**
themes.
...
file to an **RStudio** `.rstheme` file:
...
This function writes the `.rstheme` file, installs it and applies it when
`apply = TRUE`.

## Workflow example: from **Visual Studio Code** or **Positron** to **RStudio**

- Choose a `.json` theme file from **Visual Studio Code** or **Positron**.
- Convert and install it with `convert_to_rstudio_theme()`.
- Apply it with `rstudioapi::applyTheme()` or the **RStudio** interface.
...
- Use the Add Theme interface in the **RStudio** IDE
  (`Global Options > Appearance > Add`) to manually add `.rstheme` files that
  you have created or converted.
